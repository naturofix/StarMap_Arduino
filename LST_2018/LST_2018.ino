    ...
    //setting this variable to true resets the time to the computer current time
    //a battery keeps the time, while the arduino is off
    //leaving it as true will result in the time being set the the time the script was last uploaded, when the arduino is turned back on
    //run ths scrip once with reset_time = true to reset the time
    //run the scrip again with reset_time = false to maintain the time
    boolean reset_time = true; 
    ...
    double LEO_RA_s = 9.5;
    double LEO_RA = 10.6;
    double LEO_RA_e = 11.8;

    double VIR_RA_s = 11.8;
    double VIR_RA = 13.2;
    double VIR_RA_e = 14.6;

    double LIB_RA_s = 14.6;
    double LIB_RA = 15.3;
    double LIB_RA_e = 16.2;
    
    
    double SCO_RA_s = 16.2;
    double SCO_RA = 16.6;
    double SCO_RA_e = 16.6;
    
        
    double OPH_RA_s = 16.6;
    double OPH_RA = 17.2;    
    double OPH_RA_e = 17.8;
    
    
    double SGR_RA_s = 17.8;
    double SGR_RA = 19.1;
    double SGR_RA_e = 20.1;
    
    double CAP_RA_s = 20.1;
    double CAP_RA = 21.1;
    double CAP_RA_e = 22;
    
    double AQR_RA_s = 22;
    double AQR_RA = 22.4;
    double AQR_RA_e = 23.4;
    
    double PSC_RA_s = 23.4;
    double PSC_RA = 0.5;
    double PSC_RA_e = 1.7;
...
//    Serial.println();
    ...
    double sun_const(String(Name), double(s),double(e), double(sun)){
      if(sun < 0){
        sun = sun + 24;
      }
      if(sun > 24){
...
// int LAMP = 3;
   int LAMP = 22;
    ...
    float R2 = 9400;
    int value = 0;
void bubbles(float LST, float LST_sun_rise, float LST_sun_set){
  int time_on = 3;
  byte hit = 0;
  if(LST > LST_sun_rise && LST < LST_sun_rise + time_on){
    digitalWrite(LAMP, HIGH);
    hit = 1;
  }
  if(LST < LST_sun_set && LST < LST_sun_set - time_on){
    digitalWrite(LAMP, HIGH);
    hit = 1;
  }
  if(hit == 0){
...
void loop () {
      DateTime now = RTC.now();
       ...
       if(Serial.available() > 0){
         ...
         test_var = test_var + 1;
         char serial_entry[] = "test";
         //serial_entry = 
         Serial.readBytes(serial_entry,10);
         Serial.println(serial_entry);
         Serial.println();
       }
       //double JD0;
      //double sun_H;
   ...
   if(switchstate == LOW){
     bubbles(LST,LST_sun_rise,LST_sun_set);
     int sun_vis = visible("SUN",SUN,sun_RA,sun_dec,latitude,LST);
     Serial.print('Sun Visible : ');
     Serial.println(sun_vis);
     if(count <= 1){
...
//       visible("ORI",ORI,ORI_RA,ORI_dec,latitude,LST);
      ...
      digitalWrite(LAMP, LOW);
       ...
       int ARI_sun = sun_const("ARI",ARI_RA_s,ARI_RA_e,sun_RA);
       int TAU_sun = sun_const("TAU",TAU_RA_s,TAU_RA_e,sun_RA);
       int GEM_sun = sun_const("GEM",GEM_RA_s,GEM_RA_e,sun_RA);
       int CNC_sun = sun_const("CNC",CNC_RA_s,CNC_RA_e,sun_RA);
       int LEO_sun = sun_const("LEO",LEO_RA_s,LEO_RA_e,sun_RA);
       int VIR_sun = sun_const("VIR",VIR_RA_s,VIR_RA_e,sun_RA);
       int LIB_sun = sun_const("LIB",LIB_RA_s,LIB_RA_e,sun_RA);
       int SCO_sun = sun_const("SCO",SCO_RA_s,SCO_RA_e,sun_RA);
       int OPH_sun = sun_const("OPH",OPH_RA_s,OPH_RA_e,sun_RA);
       int SGR_sun = sun_const("SGR",SGR_RA_s,SGR_RA_e,sun_RA);
       int CAP_sun = sun_const("CAP",CAP_RA_s,CAP_RA_e,sun_RA);
       int AQR_sun = sun_const("AQR",AQR_RA_s,AQR_RA_e,sun_RA);
       int PSC_sun = sun_const("PSC",PSC_RA_s,PSC_RA_e,sun_RA);
       
       if(ARI_sun == 1){digitalWrite(ARI, HIGH);}       
       if(TAU_sun == 1){digitalWrite(TAU, HIGH);}
       if(GEM_sun == 1){digitalWrite(GEM, HIGH);}
       if(CNC_sun == 1){digitalWrite(CNC, HIGH);}
       if(LEO_sun == 1){digitalWrite(LEO, HIGH);}
       if(VIR_sun == 1){digitalWrite(VIR, HIGH);}
       if(LIB_sun == 1){digitalWrite(LIB, HIGH);}
       if(SCO_sun == 1){digitalWrite(SCO, HIGH);}
       if(OPH_sun == 1){digitalWrite(OPH, HIGH);}
       if(SGR_sun == 1){digitalWrite(SGR, HIGH);}
       if(CAP_sun == 1){digitalWrite(CAP, HIGH);}
       if(AQR_sun == 1){digitalWrite(AQR, HIGH);}
       if(PSC_sun == 1){digitalWrite(PSC, HIGH);}
